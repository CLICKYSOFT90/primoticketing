...
class Service extends BaseModel
{
    ...
    public static function actionButtons($data){
        $return = '<div class="btn-group" role="group">
                        <a class="btn" href="'.route('services.show', $data->id).'" title="View">
    ...
    public static function syncServices(){
        ...
        try {
            ...
            if(!empty($decodedResponse->data) && (!empty($decodedResponse->data->services->page_info))) {
                $pageInfo = $decodedResponse->data->services->page_info;
                ...
                if($total_count > 0){
                    for($i=1;$i<=$totalPages;$i++){
                        ...
                        $decodedResponse = json_decode($response->getBody()->getContents(), false, JSON_PRETTY_PRINT);
                        if(!empty($decodedResponse->data) && (!empty($decodedResponse->data->services->entities))){

                            foreach ($decodedResponse->data->services->entities as $data){
                                self::updateOrCreate(
                                    ['sonar_id' => $data->id],
                                    [
                                        'sonar_id' => $data->id,
                                        ...
                                        'upload_speed_kilobits_per_second' => (!empty($data->data_service_detail) ? $data->data_service_detail->upload_speed_kilobits_per_second : null),
                                        'download_speed_kilobits_per_second' => (!empty($data->data_service_detail) ? $data->data_service_detail->download_speed_kilobits_per_second : null),
                                        'billing_frequency' => (!empty($data->data_service_detail) ? $data->data_service_detail->billing_frequency : null),
                                        'technology_code' => (!empty($data->data_service_detail) ? $data->data_service_detail->technology_code : null),
                                        'active' => $data->enabled,
                                    ]
                                );
                            }
                    ...
                    return ['success'=>true,'msg'=>"Services synced successfully.."];
                }else{
